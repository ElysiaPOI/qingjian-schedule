...
// 国务院办公厅《关于2026年部分节假日安排的通知》
// 2027 年安排尚未公布，因此不在这里推测；公布后按同一格式补充即可。
const holidays: Record<string, HolidayInfo> = {
  ...
  "2026-01-04": { name: "元旦调休", kind: "workday" },

  "2026-02-14": { name: "春节调休", kind: "workday" },
  "2026-02-15": { name: "春节", kind: "holiday" },
  "2026-02-16": { name: "春节", kind: "holiday" },
  ...
  "2026-02-28": { name: "春节调休", kind: "workday" },
  ...
  "2026-10-10": { name: "国庆节调休", kind: "workday" },
}
...
export function holidayForDate(date: Date): HolidayInfo | null {
  return holidays[localDateKey(date)] ?? null
}
